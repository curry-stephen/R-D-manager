  ...
  <div>
    ...
    <el-dialog v-model="EditdialogVisible" title="编辑" width="30%">
      <el-form :model="editform" label-width="120px">
        ...
        <el-form-item label="库存">
          <el-input v-model="editform.stock" style="width: 80%"></el-input>
        </el-form-item>
        <el-form-item label="生产日期">
          <el-input
...
export default {
  ...
  methods: {
    ...
    addsave() {
      ...
      request.post("/druginfo", this.form).then((res) => {
        if (res.falg) {
          this.$message.success(res.msg);
        } else {
          this.$message.error(res.msg);
    ...
    Editsave() {
      ...
      request.put("/druginfo", this.editform).then((res) => {
        if (res.falg) {
          this.$message.success(res.msg);
        } else {
          this.$message.error(res.msg);
    ...
    //删除
    handleDelete(row) {
      request.delete("/druginfo/" + row+"/"+this.name).then((res) => {
        if (res.falg) {
          this.$message.success(res.msg);
        } else {
          this.$message.error(res.msg);
